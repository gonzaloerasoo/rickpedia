...
export class EpisodesService {
  ...
  getAllEpisodes(): Observable<Episode[]> {
    return this.http.get<{ info: { pages: number }; results: Episode[] }>(this.baseUrl).pipe(
      switchMap((res) => {
        const totalPages = res.info.pages;
        const requests = Array.from({ length: totalPages }, (_, i) =>
          this.http.get<{ results: Episode[] }>(`${this.baseUrl}?page=${i + 1}`)
        );
  ...
  getEpisodeById(id: string): Observable<Episode> {
    return this.http.get<Episode>(`${this.baseUrl}/${id}`).pipe(catchError(this.handleError));
  }

  getEpisodeByName(name: string): Observable<Episode | null> {
    return this.http.get<{ results: Episode[] }>(`${this.baseUrl}/?name=${name}`).pipe(
